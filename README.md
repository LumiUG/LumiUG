![Header](https://github.com/LumiUG/LumiUG/blob/main/images/Header.png)

### About me and me and me and me-
Yo!! I'm "ug", an [indie developer](https://ugdev.itch.io) with many skills from Spain! *(Catalonia)*<br>
I mainly program in **C#**, **Python** (rarely), and do random stuff with **HTML/CSS/JS**.<br>

Backend work is my specialty, but I've been exploring with **UI/UX** and presentation for a while now. Plus I should be getting into **shaders** soon enough.

<img align="right" width="200" src="https://github.com/LumiUG/LumiUG/blob/main/images/ZilluWave.png" alt="Lumi! Art by Zilluzion." />

* Intermediate Unity Programmer!
* Puzzle / Game Designer.
* I [love videogames](https://backloggd.com/u/ugdev)!!!
* 2D Beginner artist, and getting into [music](https://ugdev.bandcamp.com).
* I also own a [blog](https://ugdev.xyz/blog)! Even if a bit barebones...
* Space bird :3

Currently working on [A Directionless Cycle](https://ugdev.xyz/adc) and [Polymetria](https://www.hajimeli.net/) (which is on hold), with some side-projects like [Gravix EOS](https://ugdev.itch.io/gravix-eos) and a mysterious [client](https://ugdev.itch.io/eos-cli)...

### GitHub Stats!!!
| Stats | Languages |
| :---: | :---: |
| <img src="https://github-readme-stats.vercel.app/api?username=LumiUG&title_color=DA7885&text_color=E1B2A2&show_icons=true&icon_color=BB8470&bg_color=170F0C&hide_border=true&locale=en"><br> | <img src="https://github-readme-stats.vercel.app/api/top-langs?username=LumiUG&theme=gruvbox&show_icons=true&hide_border=true&locale=en&layout=compact"> |
